...
<body class="antialiased bg-white text-ink font-sans">
        ...
        <div class="max-w-7xl mx-auto px-5 lg:px-8">
            <div class="flex items-center justify-between h-16 lg:h-20">
                ...
                <nav class="hidden lg:flex items-center gap-8 text-sm text-white/80">
                    ...
                    <a href="#categories" class="hover:text-white transition">Workforce</a>
                    <a href="#nationalities" class="hover:text-white transition">Nationalities</a>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="#contact" class="lg:hidden inline-flex items-center rounded-full border border-white/20 text-white text-sm font-medium px-4 py-2">
                        Contact
                    </a>
                </div>
            </div>
    ...
    <section id="top" class="hero-bg relative overflow-hidden">
        <div class="max-w-7xl mx-auto px-5 lg:px-8 pt-16 pb-20 lg:pt-24 lg:pb-28 relative">
            <div class="max-w-3xl">
                ...
                <p class="mt-5 text-white/70 text-base lg:text-lg leading-relaxed">
                    Manpower Supply KSA is a trusted manpower and staffing solutions provider across Riyadh, Jeddah, Dammam, Jubail, Yanbu, NEOM & all major cities — supplying skilled, semi-skilled, and unskilled workforce to main companies, subcontractors, facility management, hospitality, healthcare, and construction sectors.
                </p>
                ...
                <div class="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-4">
                    @foreach ([
                        ['60+', 'Job Categories'],
                        ['24-48h', 'Fast Deployment'],
                        ['24/7', 'Support'],
                        ['7+', 'Nationalities'],
                    ] as [$stat, $label])
                        <div class="rounded-xl bg-white/5 border border-white/10 px-4 py-4">
                            <p class="text-brand-300 text-xl lg:text-2xl font-bold">{{ $stat }}</p>
    ...
    {{-- ============ ABOUT ============ --}}
    <section id="about" class="max-w-7xl mx-auto px-5 lg:px-8 py-16 lg:py-24">
        <div class="grid lg:grid-cols-2 gap-10 items-center">
            <div>
                ...
                <div class="mt-6 grid sm:grid-cols-2 gap-4">
                    @foreach ([
                        'Valid Iqama & Ready to Join Candidates',
                        'Fast Deployment - Within 24-48 Hours',
                        'Daily & Monthly Supply Available',
                        'Document Verified & Medically Fit Workers',
                        '24/7 Support and Replacement Guarantee',
                        'Ready to Transfer (Tanazul) Candidates',
                    ] as $point)
                        <div class="flex items-start gap-3">
                            <span class="mt-0.5 flex-shrink-0 w-5 h-5 rounded-full bg-brand-100 text-brand-700 flex items-center justify-center text-xs">✓</span>
            ...
            <div class="rounded-2xl bg-brand-950 p-8 lg:p-10 text-white">
                <h3 class="font-semibold text-lg mb-6">Service Coverage</h3>
                <div class="flex flex-wrap gap-2">
                    @foreach (['Riyadh','Jeddah','Dammam','Jubail','Yanbu','NEOM','All Major Cities'] as $city)
                        <span class="rounded-full bg-white/10 border border-white/15 text-sm px-4 py-1.5">{{ $city }}</span>
                    @endforeach
                </div>

                <h3 class="font-semibold text-lg mt-8 mb-4">Industries We Serve</h3>
    ...
    <section id="categories" class="bg-gray-50 py-16 lg:py-24">
        <div class="max-w-7xl mx-auto px-5 lg:px-8">
            <div class="text-center max-w-2xl mx-auto">
                ...
                <p class="mt-3 text-gray-600">From construction and hospitality to domestic and security staff — we cover it all.</p>
            </div>
            ...
            <div class="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @foreach ([
                    ...
                    ['title' => 'Construction', 'items' => ['Mason', 'Steel Fixer', 'Shuttering Carpenter', 'Painter', 'Electrician', 'Plumber', 'Welder', 'AC Technician', 'Scaffolder']],
    ...
    {{-- ============ NATIONALITIES ============ --}}
    <section id="nationalities" class="max-w-7xl mx-auto px-5 lg:px-8 py-16 lg:py-24">
        <div class="text-center max-w-2xl mx-auto">
            <p class="text-brand-600 font-semibold text-sm uppercase tracking-wider">Nationalities Available</p>
            <h2 class="mt-2 text-2xl lg:text-3xl font-bold text-ink">Diverse, Verified & Ready</h2>
            <p class="mt-3 text-gray-600">All candidates come with valid Iqama and are ready for transfer (Tanazul).</p>
        </div>

        <div class="mt-10 flex flex-wrap justify-center gap-3">
            @foreach (['Filipino','Indonesian','Bangladeshi','Nepali','Indian','Pakistani','Ugandan','& More'] as $nat)
                <span class="rounded-full border border-brand-200 bg-brand-50 text-brand-800 text-sm font-medium px-5 py-2">{{ $nat }}</span>
            @endforeach
        </div>
    </section>

    {{-- ============ WHY CHOOSE US ============ --}}
    <section id="why-us" class="bg-ink py-16 lg:py-24">
        <div class="max-w-7xl mx-auto px-5 lg:px-8">
            <div class="text-center max-w-2xl mx-auto">
                <p class="text-brand-400 font-semibold text-sm uppercase tracking-wider">Why Companies Choose Us</p>
                <h2 class="mt-2 text-2xl lg:text-3xl font-bold text-white">Built for Speed, Trust & Compliance</h2>
            </div>
            ...
            <div class="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ([
                    ...
                    ['icon' => '🏗️', 'title' => 'Project & Urgent Ready', 'desc' => 'Supporting long-term projects and urgent daily requirements.'],
                ] as $card)
                    <div class="rounded-2xl bg-white/5 border border-white/10 p-6">
                        <div class="w-10 h-10 rounded-lg bg-brand-500/20 flex items-center justify-center text-lg mb-4">{{ $card['icon'] }}</div>
                        <h3 class="text-white font-semibold mb-2">{{ $card['title'] }}</h3>
                        <p class="text-white/60 text-sm leading-relaxed">{{ $card['desc'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
                ...
                <div class="p-8 lg:p-14 text-white">
                    ...
                    <div class="mt-8 space-y-4">
                        ...
                        <div class="flex items-center gap-3">
                            <span class="w-10 h-10 rounded-full bg-white/15 flex items-center justify-center">📍</span>
                            <span class="text-sm">
                                <span class="block text-white/70">Location</span>
                                <span class="font-semibold">Riyadh, KSA — Serving All Over Saudi Arabia</span>
                            </span>
                        </div>
                    </div>
